...
export class SubmissionController implements ISubmissionController {
  ...
  async getSubmissions(
    ...
    try {
      ...
      res
        ...
        .send(
          buildResponseSuccess(results, SubmissionMessage.SUBMISSION_RETRIEVED)
        );
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          ...
          .send(
            buildResponseError(
              error.message,
              SubmissionMessage.SUBMISSION_NOT_FOUND
            )
          );
  ...
  async getSubmissionById(
    ...
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(
          buildResponseSuccess(result, SubmissionMessage.SUBMISSION_RETRIEVED)
        );
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
  ...
  async getSubmissionsByUserId(
    ...
    try {
      ...
      const results = await this.submissionService.getSubmissionsByUserId(
        req.user.id
      );
      res
        .status(StatusCode.OK)
  ...
  async createSubmission(
    ...
    try {
      ...
      res
        ...
        .send(
          buildResponseSuccess(result, SubmissionMessage.SUBMISSION_CREATED)
        );
    } catch (error: any) {
  ...
  async updateSubmission(
    req: Request<{ id: string }, {}, UpdateSubmissionRequest>,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(
          buildResponseSuccess(result, SubmissionMessage.SUBMISSION_UPDATED)
        );
    } catch (error: any) {
      if (error instanceof BadRequestError) {
        res
  ...
  async updateSubmissionStatus(
    req: Request<{ id: string }, {}, UpdateSubmissionStatusRequest>,
    res: Response,
    next: NextFunction
  ): Promise<void> {
    try {
      ...
      const result = await this.submissionService.updateSubmissionStatus(
        submissionId,
        req.body.status
      );
      res
        ...
        .send(
          buildResponseSuccess(
            result,
            SubmissionMessage.SUBMISSION_STATUS_UPDATED
          )
        );
    } catch (error: any) {
      if (error instanceof BadRequestError) {
