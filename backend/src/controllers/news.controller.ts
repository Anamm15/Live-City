...
export class NewsController implements INewsController {
  ...
  async getNews(
    ...
    try {
      ...
      res
        ...
        .send(buildResponseSuccess(results, NewsMessage.NEWS_RETRIEVED));
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          .status(StatusCode.NOT_FOUND)
          .send(buildResponseError(error.message, NewsMessage.NEWS_NOT_FOUND));
      } else {
        res
  ...
  async getNewsById(
    ...
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(buildResponseSuccess(result, NewsMessage.NEWS_RETRIEVED));
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          .status(StatusCode.NOT_FOUND)
  ...
  async createNews(
    ...
    try {
      ...
      res
        ...
        .send(buildResponseSuccess(result, NewsMessage.NEWS_CREATED));
    } catch (error: any) {
      if (error instanceof BadRequestError) {
        res
          .status(StatusCode.BAD_REQUEST)
          .send(
            buildResponseError(error.message, CommonMessage.FILE_NOT_FOUND)
          );
      } else {
        res
          .status(StatusCode.INTERNAL_SERVER_ERROR)
          .send(
            buildResponseError(error.message, NewsMessage.NEWS_CREATE_FAILED)
          );
      }
    }
  }
  ...
  async updateNews(
    ...
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(buildResponseSuccess(result, NewsMessage.NEWS_UPDATED));
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          .status(StatusCode.NOT_FOUND)
        ...
        res
          .status(StatusCode.INTERNAL_SERVER_ERROR)
          .send(
            buildResponseError(error.message, NewsMessage.NEWS_UPDATE_FAILED)
          );
      }
    }
  }
    ...
    try {
      ...
      if (error instanceof NotFoundError) {
        ...
        res
          .status(StatusCode.INTERNAL_SERVER_ERROR)
          .send(
            buildResponseError(error.message, NewsMessage.NEWS_DELETE_FAILED)
          );
      }
    }
  }
  ...
  async getNewsComments(
    ...
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(
          buildResponseSuccess(results, NewsMessage.NEWS_COMMENT_RETRIEVED)
        );
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
  ...
  async createNewsComment(
    ...
    try {
      ...
      const result = await this.newsService.createNewsComment(
        newsId,
        req.user.id,
        req.body
      );
      res
        .status(StatusCode.CREATED)
        .send(buildResponseSuccess(result, NewsMessage.NEWS_COMMENT_CREATED));
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          .status(StatusCode.NOT_FOUND)
  ...
  async getNewsReactions(
    ...
    try {
      ...
      res
        .status(StatusCode.OK)
        .send(
          buildResponseSuccess(results, NewsMessage.NEWS_REACTION_RETRIEVED)
        );
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
  ...
  async createNewsReactions(
    ...
    try {
      ...
      const result = await this.newsService.createNewsReactions(
        newsId,
        req.user.id
      );
      res
        .status(StatusCode.CREATED)
        .send(buildResponseSuccess(result, NewsMessage.NEWS_REACTION_CREATED));
    } catch (error: any) {
      if (error instanceof NotFoundError) {
        res
          .status(StatusCode.NOT_FOUND)
