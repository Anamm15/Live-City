...
export class FacilityService implements IFacilityService {
   ...
   async createFacility(data: CreateFacilityRequest, file: Express.Multer.File): Promise<FacilityResponse> {
      let cloudinaryResult: any | null = null;
      return await this.prisma.$transaction(async (tx: Prisma.TransactionClient) => {
         try {
            const newFacility = await this.facilityRepository.createFacility(data, tx);
            const newFilename = generateFilename(FileableType.FACILITY, newFacility.id);
